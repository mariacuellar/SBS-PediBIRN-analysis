...
We begin by loading the packages used throughout the analysis.
...
### Constructing the Clustering Variables

We now map the raw spreadsheet columns into the variables used for clustering. We also create the derived severity and injury indicators used later in the analysis.
...
```{r dendrogram}
#| fig-cap: "Hierarchical clustering dendrogram based on Gower distance."
...
#### Age Categories Across the Two Main Clusters
...
```{r age-plot}
#| fig-cap: "Age-category distribution across the two main hierarchical clusters."
...
\begin{aligned}
A &\rightarrow S \\
A &\rightarrow X_j, \quad j = 1, \dots, p \\
...
![Figure: Causal DAG for Boos critique](mermaid-diagram.png){fig-width=6, fig-align="center"}
