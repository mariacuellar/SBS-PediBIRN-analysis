...
## FCI Search for Model A

We now run a focused FCI search on the same four Model A variables as a data-driven alternative to the hand-specified mediation model. Unlike the mediation analysis, FCI allows for latent confounding and returns a partial ancestral graph rather than a fully committed DAG.

The FCI algorithm is a causal discovery procedure that uses patterns of conditional independence in the observed data to learn a set of causal relationships that are compatible with the data while allowing for the possibility of unmeasured confounding. Rather than testing one prespecified mediation pathway, it searches over the observed variables and returns a partial ancestral graph, or PAG, which represents a class of causal graphs that fit the data under the algorithm's assumptions. The output can include directed edges, partially oriented edges, and edges with unresolved endpoints, so it usually identifies some causal features without committing to a single fully determined DAG. Its main assumptions are that the observed conditional independences arise from an underlying causal graph, that the data satisfy the Markov and faithfulness assumptions with respect to that graph, and that the conditional independence tests used by the algorithm are reliable for the sample at hand.

```{r fci-model-a}
library(pcalg)

model_a_fci_data <- oph_subset |>
  select(isolated_inertial_injury, diffuse_HIE, extra_axial_blood, severe_retinal_injury) |>
  na.omit()

fci_model_a <- fci(
  suffStat = list(dm = as.matrix(model_a_fci_data), adaptDF = FALSE),
  indepTest = binCItest,
  alpha = 0.05,
  labels = names(model_a_fci_data),
  skel.method = "stable",
  verbose = FALSE
)

fci_amat <- as(fci_model_a, "matrix")

decode_pag_edge <- function(mark_ij, mark_ji) {
  left_mark <- dplyr::case_when(
    mark_ji == 1 ~ "o",
    mark_ji == 2 ~ "<",
    mark_ji == 3 ~ "-",
    TRUE ~ " "
  )
  right_mark <- dplyr::case_when(
    mark_ij == 1 ~ "o",
    mark_ij == 2 ~ ">",
    mark_ij == 3 ~ "-",
    TRUE ~ " "
  )
  paste0(left_mark, "-", right_mark)
}

fci_edges_tbl <- bind_rows(lapply(seq_len(ncol(fci_amat) - 1), function(i) {
  bind_rows(lapply(seq((i + 1), ncol(fci_amat)), function(j) {
    mark_ij <- fci_amat[i, j]
    mark_ji <- fci_amat[j, i]
    ...
    tibble(
      node_1 = colnames(fci_amat)[i],
      node_2 = colnames(fci_amat)[j],
      pag_edge = decode_pag_edge(mark_ij, mark_ji),
      interpretation = case_when(
        decode_pag_edge(mark_ij, mark_ji) == "o->" ~ "compatible with a directed edge into node_2, but with uncertainty at node_1",
        decode_pag_edge(mark_ij, mark_ji) == "o-o" ~ "adjacent, but with unresolved orientation at both ends",
        TRUE ~ "partially oriented PAG edge"
      )
    )
  }))
}))

kable(fci_edges_tbl, caption = "Table 5. FCI edge summary for the Model A variable set")
```

Table 5 translates the FCI output into edge marks that are easier to read than the raw PAG adjacency matrix. In this subset, the FCI search preserves links from all three putative predictors to severe retinal injury, while also leaving some orientation uncertainty rather than endorsing the mediation model as a uniquely identified DAG.

We now show the same FCI result as a simple partial ancestral graph for the Model A variable set.

![FCI partial ancestral graph for the Model A variable set.](figures-mediation-analysis/modelA-fci-pag.svg)
...
## FCI-Based Comparison of Models A-E

We now compare Models A-E using FCI in the way the algorithm is actually designed to be used. FCI does not produce a single global score for a hand-specified mediation DAG, so instead we compare each proposed model against the PAG estimated on that model's variable set and count how many expected arrows are strongly supported, only partially supported, absent, or contradicted.
...
fci_model_results <- lapply(seq_len(nrow(model_specs)), function(i) {
  spec <- model_specs[i, ]
  vars <- c(spec$exposure, spec$mediators[[1]], "severe_retinal_injury")
  ...
  fit <- fci(
    suffStat = list(dm = as.matrix(dat), adaptDF = FALSE),
    indepTest = disCItest,
    alpha = 0.05,
    labels = vars,
    skel.method = "stable",
...
fci_model_summary_tbl <- fci_edge_assessment_tbl |>
  ...
  left_join(
    tibble(
      model = names(fci_model_results),
      n_complete = vapply(fci_model_results, function(x) nrow(x$data), integer(1))
    ),
    by = "model"
  ) |>
  mutate(
...
kable(
  ...
  caption = "Table 6. FCI-based compatibility summary for Models A-E"
)
```

Table 6 uses FCI in a model-comparison role without forcing it into a mediation-estimator framework it was not designed for. Under this criterion, Model A is the most compatible with the estimated PAG, with three strongly supported expected arrows, one partially supported arrow, and only one absent arrow. By contrast, Model B performs poorly because most of its expected arrows are absent from the FCI graph on this subset.
